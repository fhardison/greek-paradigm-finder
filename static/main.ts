...
var url : string = "static/morpheus_form_lemma.txt";
var data : { [id: string] : {[id: string] : string[]} } = {};
...
const search_box = <HTMLInputElement>  document.getElementById("search-box");

function genderNum(x) : number {
	if (x == "m") {
		return 0;
	} else if (x == "f") {
		return 1;
...
function fillCell(xs) : HTMLElement {
...
function sortByTarget(xs , names: string[], featureGetter : (arg: string) => string, dropEmpty: boolean, debug: boolean, step: string) : string[][]{
...
function display_infinitives(xs) : void {
...
function searchData() : void {
	let lemma :string = search_box.value.normalize("NFC");
	data_display.innerHTML = "";
	if (!(lemma in data)) {
		let res = document.createElement('p');
		res.textContent = "Not found";
		data_display?.appendChild(res);
	} else {
		let words : { [id: string] : string[] } = data[lemma];
